...
axle_axis=[
	motor_mount_translation[0]-gear_separation,
...
idler_top_centre=idler_axis+[2,idler_long_top,0];

idler_clip_recess_thickness=5;
idler_clip_recess_width=20;
idler_clip_recess_depth=5;
idler_clip_recess_angle=15;
...
module block_holes()
{
	...
	// Remove an unsupported dag over the filament hole.
	#translate(axle_axis+[-2.5,6.5,5])
	rotate(20)
	cube([5,5,10],center=true);

	// Holes for the screws for the clip to clip onto.

	for (i=[-1,1])
	translate([2,wade_block_height,wade_block_depth/2+i*7])
	rotate(7.5)
	rotate([-90,0,0])
	translate([0,0,-8])
	cylinder(r=1,h=100);
	...
	translate([wade_block_width/2,motor_mount_translation[1]+5,motor_mount_thickness])
	cube([wade_block_width/2+1,
		wade_block_height-motor_mount_translation[1]-5+1,
		wade_block_depth-motor_mount_thickness+1]);
...
%			rotate([0,180,0])
			...
			rotate(-15)
			translate([-4,0,9.5])
			cube([8,40,wade_block_depth-9.5+1]);
...
module wadeidler() 
{
	difference()
	{
		...
		// Recess for bulldog clip.
#		translate(idler_top_centre+[-2,-idler_clip_recess_depth,-idler_clip_recess_width/2])
		rotate(90+idler_clip_recess_angle)
		cube([20,idler_clip_recess_thickness,idler_clip_recess_width]);
...
//%import_stl ("wade.stl");
//translate([-5,45,wade_block_depth/2])
//rotate(clip_angle/2)
//clip();
//
//clip_height=12;
...
//for(clip=[-1,1])
//{
//translate([-clip_separation/2*clip,0,0])
//rotate([0,0,clip*clip_angle/2])
//translate([-clip*clip_thickness/2,clip_height/2,0])
//cube([clip_thickness,clip_height,20],center=true);
//}
//}
